...
class Robot_player(Robot):
    ...
    def step(self, sensors, sensor_view=None, sensor_robot=None, sensor_team=None):
        ...
        if (self.trial < 500):
            if self.iteration % self.it_per_evaluation == 0:
                    ...
                    if self.score > self.best_score:
                        print("best param : ",self.bestParam," best score : ", self.best_score," param : ", self.param," score : ",self.score)
                        self.bestParam = self.param
                        self.best_score = self.score
                    self.param = [random.randint(-1, 1) for i in range(8)]
                    if self.trial == self.max_trial:
                        print("best param : ", self.bestParam," best score : ",self.best_score," try : ",self.trial)
        ...
        #print("prev_log_tr : ",self.prev_log_tr)
        tr_it = self.log_sum_of_translation - self.prev_log_tr
        self.prev_log_tr = self.log_sum_of_translation
        #print("tr it : ", tr_it," prev_log_tr : ", self.prev_log_tr)


        rot_it = self.log_sum_of_rotation - self.prev_log_rot    
        self.prev_log_rot = self.log_sum_of_rotation


        self.score += tr_it * (1-abs(rot_it))
